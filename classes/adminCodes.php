...
class AdminManager
{
    ...
    public function adminLogin()
    {
        ...
        if (!empty($errors)) {
            ...
            //if it returns true user exist and user activated
            if ($user = $this->auth->login($username, $password)) {
                $_SESSION['adminId'] = $user['adminId'];
                $_SESSION['logged'] = true;

                header("location:home.php?user=" . base64_encode($user['username']) . "&name=" . base64_encode($res[name]) . "&adminstatus=" . base64_encode($res['status']));
            } else {
                if ($user['status'] == 3) {
